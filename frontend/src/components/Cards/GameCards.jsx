...
function GameCards({ gameDetails, joined }) {
  const formattedDateTime = gameDetails.date
    ? formatDateTime(gameDetails.date)
    : "";
  ...
  useEffect(() => {
    ...
    if (
      gameDetails.playersList &&
      gameDetails.playersList.includes(auth.userAuth.id)
    ) {
  ...
  async function handleGameJoin() {
    ...
    try {
      const joinGameRequest = await joinGameApi(
        auth.userAuth.id,
        gameDetails._id
      );
  ...
  async function handleLeaveGame() {
    try {
      const leaveGameRequest = await leaveGameApi(
        auth.userAuth.id,
        gameDetails._id
      );
    ...
    <Card className={styles.game_card}>
      ...
      <CardContent>
        <Box sx={{ display: "flex", 
          ...
          >
            {gameDetails.eventTitle.charAt(0).toUpperCase() +
              gameDetails.eventTitle.slice(1)}
          </Typography>
          <Typography
            variant="body2"
            sx={{
          ...
          >
            {gameDetails.sportType === "tabletennis"
              ? "Table-Tennis"
      ...
      {gameDetails.status !== "finished" ? (
        ...
        <Typography
          variant="body1"
          sx={{
            color: gameDetails.winners.includes(auth.userAuth.id)
              ? "green"
              : "red",
            fontWeight: "bold",
            textAlign: "center",
